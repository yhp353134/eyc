...
<body>
		<div style="width: 100%;margin-top: 10px;"></div>
			<table cellpadding="0" cellspacing="0">
				<form action="sourceGoods/orderList.do"  method="post"   id="userPost">
				<tr>
					<td style="text-align: left;">
						...
						<span>
							<input style="margin-top: 10px;" autocomplete="off" id="nav-search-input" type="text" name="orderVin" placeholder="请输入VIN码" value="${pd.orderVin }"/>
						</span>
						&nbsp;&nbsp;
						起始地:
			...
			<table id="table_bug_report" class="table table-striped table-bordered table-hover">
				<thead>
					<tr>
						...
						<th class="center" >发车人</th>
						<th class="center" >收车人</th>
				...
				<tbody>
					<c:forEach items="${orderList }"  var="order">
						<tr>
							...
							<td class="center">${order.MODLE_NAME }</td>
							...
							<td class="center">${order.PAY_NAME }</td> 
							<td class="center">${order.RE_NAME }</td>
...
function detailOrder(obj) {
	var orderId = $(obj).parent().parent().find("input[type='hidden']").val();
	var price = $(obj).parent().parent().find("td").eq(3).html();
	if (checkNullStr(price)) {
		price = 0;
	}
	top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="确认交车";
	 diag.URL =globalContextPath+ '/sourceGoods/orderDealCar.do?price='+price+'&orderId='+orderId;
	 diag.Width = 900;
	 diag.Height = 550;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
			 top.jzts();
			window.location.href=globalContextPath+"/sourceGoods/orderList.do";
		}
		diag.close();
	 };
	 diag.show();
...
function cancelSource(obj) {
	...
	top.jzts();
	 ...
	 diag.Drag=true;
	 diag.Title ="订单撤销";
	 diag.URL =globalContextPath+ '/sourceGoods/goToOrderPriceRevoke.do?sourceId='+sourceId;
	 diag.Width = 500;
	 diag.Height = 200;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
