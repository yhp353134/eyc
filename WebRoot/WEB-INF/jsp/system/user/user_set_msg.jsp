...
<body>
		<div style="width: 100%;margin-top: 10px;"></div>
			<table>
				<tr>
					<form action="user/userSetMsg.do"  method="post"  id="userPost">
					<td>
						<span>
					...
					<c:if test="${orgType eq '10120001' }">
						<td>
							...
							<input type="text" name="orgName"  autocomplete="off"  id="nav-search-input"   value="${pd.orgName }" placeholder="请选择服务站"  style="margin-top: 10px;"  readonly="readonly" onclick="chooseorg()"/>
						</td>
					</c:if>
					<td>
					...
					<c:forEach items="${userList }"  var="user">
						<tr>
							...
							<td class="center" style="cursor: pointer;">
								<label><input type="radio"  class="userId" name="userId" value="${user.USER_ID}"  onclick="chooseUserById(this)"/><span class="lbl"></span></label>
							</td>
...
function chooseUserById(e) {
	var thisValue = $(e).val();
	$.ajax({
		type: "POST",
		url: '<%=path%>/login_login.do',
    	data: {
    		userId:thisValue,
    		tm:new Date().getTime()
    	},
		...
		success: function(data){
			if("success" == data.result){
				top.jzts();
				top.location.href="<%=path%>/main/post/admin.do?changeMenu="+data.postId;
			} else if("hasMorePost" == data.result){
				openDiv();
			} else {
				bootbox.alert("用户信息有误，不能切换");
			}
		}
	});
}
function openDiv() {
		 ...
		 diag.CancelEvent = function(){
	        if(diag.innerFrame.contentWindow.document.getElementById('postListDiv').style.display == 'none'){
				...
				top.location.href="<%=basePath%>main/post/admin.do?changeMenu="+postId;
				 diag.close();
			 } else {
				 diag.close();
			 }
