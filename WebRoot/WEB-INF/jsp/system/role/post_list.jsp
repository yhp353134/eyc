...
<div style="width: 100%;position: relative;">
	<div style="width: 96%;position: relative;left:2%;">
		<div style="width: 100%;position: relative;">
			<table>
				<tr>
					<form action="role/postList.do" method="post" id="postMainForm">
					<td align="center">
						<span>
							<input style="width: 130px;margin-top: 10px;" autocomplete="off" id="nav-search-input" type="text" name="postNameList" value="${postNameList }"  placeholder="请输入职位名称" />
							<i id="nav-search-icon"></i>
						</span>
					</td>
					<td>
						<select class="chzn-select"  id="postType" name="postType"  data-placeholder="请选择职位">
							...
							<c:choose>
						  		...
						  		<c:otherwise>
						  			<option value="10120001" <c:if test="${postType eq '10120001' }">selected="selected"</c:if>>平台职位</option>
						  			<option value="10120002" <c:if test="${postType eq '10120002' }">selected="selected"</c:if>>公司职位</option>
						  		</c:otherwise>
						  </c:choose>
						</select>
					</td>
				...
				<tbody>
					<c:forEach items="${postList }" var="post">
						<tr>
							...
							<td class="center">${post.POST_NAME }</td>
							<td class="center">
				...
				<tr>
					<td style="vertical-align:top;">
						 <c:if test="${QX.add eq '1' }"> 
							<button type="buttion"  class="btn btn-success btn-small" onclick="addPost()">新增</button>
						</c:if> 
...
function addPost(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增职位";
	 diag.URL = '<%=basePath%>role/toPostAddPage.do';
	 diag.Width = 1000;
	 diag.Height = 500;
...
function editRole(obj){
	//获取roleId
	var postId= $(obj).parent().parent().find("input[type='hidden']").val();
	if(postId == null || postId == 'undefind' || postId == '') {
		bootbox.alert("职位ID为空，不能修改职位信息");
		return false;
	}
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="修改职位";
	 diag.URL = '<%=basePath%>role/jumpEditPost.do?postId='+postId;
	 diag.Width = 1000;
	 diag.Height = 500;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
