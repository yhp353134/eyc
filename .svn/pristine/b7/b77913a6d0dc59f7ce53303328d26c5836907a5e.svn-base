		...
		<div id="sidebar" class="menu-min">
				...
				<ul class="nav nav-list">
					 <li class="active" id="fhindex">
					  <a style="cursor: pointer;" onclick="siMenu('tab1_index1','lmtab1_index1','主页','login_default.do')"><i class="icon-dashboard"></i><span>后台首页</span></a>
					</li> 
					<c:forEach items="${menuList }" var="menu">
						<!-- 一级菜单 -->
						<li id="lm${menu.MENU_ID }">
							 <a style="cursor:pointer;" class="dropdown-toggle" >
								<i><img  src="${menu.MENU_ICON }" width="18px" height="18px"/></i>
							  ...
							  <ul class="submenu">
								...
								<c:forEach items="${menu.subMenu}" var="sub">
									<c:choose>
										<c:when test="${not empty sub.NEMU_URL}">
										<li id="z${sub.MENU_ID }">
											<a style="cursor:pointer;" target="mainFrame"  onclick="siMenu('z${sub.MENU_ID }','lm${menu.MENU_ID }','${sub.MENU_NAME }','${sub.NEMU_URL }')">
												<i class="icon-double-angle-right"></i>${sub.MENU_NAME }
											</a>
										</li>
										</c:when>
										<c:otherwise>
											<li><a href="javascript:void(0);"><i  ></i>${sub.MENU_NAME }</a></li>
										</c:otherwise>
									</c:choose>
								</c:forEach>
					  		</ul>
