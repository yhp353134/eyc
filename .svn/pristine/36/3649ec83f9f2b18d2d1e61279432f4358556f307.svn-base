...
<body>
		<div style="width: 100%;margin-top: 10px;"></div>
			<table>
				<tr>
					<form action="user/registUserList.do"  method="post"  id="userPost">
					<td>
						<span>
					...
					<td> 
						&nbsp;&nbsp;&nbsp;&nbsp;
						用户类型:
					 	<select name="userType"  id="userType"  style="width: 120px;margin-top: 10px;color: gray;" class="chzn-select" >
							<script type="text/javascript">setSelectOption('1013',true,'${pd.userType}')</script>
					  	</select>
					</td> 
						<td> 
						&nbsp;&nbsp;&nbsp;&nbsp;
				...
				<tbody>
					<c:forEach items="${userList }"  var="user">
						<tr>
							...
							<input type="hidden"  name="isAmont"  value="${user.IS_MAIN_COUNT }"/>
							...
							<td class="center" >
								<!-- 只有用户是待审核，同时当前用户是平台用户才可以审核 -->
								 <c:if test="${user.AUDIT_STATUS eq '10141001' and userType eq '10131001'}">
									<button class='btn btn-mini btn-info'  onclick="editUser(this)">审核</button>
								</c:if>
...
function editUser(obj){
	 ...
	 var isAmont = $(obj).parent().parent().find("input[name='isAmont']").val();
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="用户审核";
	 if (userType == '10131002') {
		 //B货主
		 diag.URL = '<%=basePath%>user/examineRegistUserOwner.do?userId='+userId;
	 } else if (userType == '10131004'){
		 //C端货主
		 diag.URL = '<%=basePath%>user/examineRegistUserPersonOwner.do?userId='+userId;
	 } else {
		 //司机
		 diag.URL = '<%=basePath%>user/examineRegistUserDriver.do?isAmont='+isAmont+'&userId='+userId;
	 }
	 diag.Width = 900;
	 diag.Height = 500;
...
function detailUser(obj){
	 ...
	 if (userType == '10131002') {
		...
		//C端货主
		 diag.URL = '<%=basePath%>user/detailRegistUserPersonOwner.do?userId='+userId;
	 } else {
		 //司机
