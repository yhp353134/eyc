					...
					<c:forEach items="${postList }" var="post">
						<tr>
							...
							<td class='center' style="width: 30px;">
								<label><input type="checkbox"  class="postName" name="postName" value="${post.POST_ID }" onclick="chooseOne(this)" <c:if test="${post.CHECKED eq '1' }">checked="checked"</c:if>/><span class="lbl"></span></label>
							</td>
...
<iframe id="valFrame"  frameborder="0" src="<%=basePath%>/user/jumpUserIframe.do?pdl=${pd.hidNameArr }"  style="margin:0 auto;width:100%;"></iframe>		
<!-- 引入 -->
